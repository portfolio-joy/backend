...
public interface ProjectDataRepository extends JpaRepository<ProjectData, String> {
	...
	@Query("SELECT projectData FROM ProjectData projectData WHERE projectData.project.id = (SELECT project.id FROM Project project WHERE project.name=:projectName AND project.user.username=:username) ORDER BY projectData.projectDataOrder")
